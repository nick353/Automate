...
3. ヒアリング終了後は必ずJSONを返す
   - 全体像を説明した後、必ず以下のJSON形式でタスク作成の準備を出力する
   - ユーザーが「作成してください」「お願いします」「この内容で作成して」などと言ったら作成開始
   - ただし、ヒアリングが完了し全体像を説明した時点で、ユーザーの確認を待たずにJSONを出力する
   - JSONを出力する際は、メッセージの最後に「この内容で作成してよろしいですか？」と確認メッセージを追加する
...
【重要：ヒアリング終了後の動作】
ヒアリングが完了し、全体像を説明した後は、必ず上記のJSON形式でタスク作成の準備を出力してください。
JSONを出力する際は、以下の形式でメッセージを構成してください：

1. 全体像の説明（タスク数、各タスクの役割など）
2. JSON形式のactions定義（```json ... ```ブロック内）
3. 最後に「この内容で作成してよろしいですか？」という確認メッセージ

例：
「全体像を説明しました。以下のタスクを作成します：

[全体像の説明]

```json
{{"actions": [...], "creating_info": {...}}}
```
