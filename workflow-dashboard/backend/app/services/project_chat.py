...
"""
    ...
    async def chat(
        ...
        try:
            ...
            system_prompt = f"""あなたはプロジェクト「{project.name}」の自動化ワークフローを管理・改善するAIアシスタントです。初心者にも伝わるように、短くやさしい言葉で説明してください。専門用語はできるだけ避け、出す場合はかんたんな言い換えも添えてください。最初に「ユーザーが用意するとスムーズなもの（目的、サイトURL、実行頻度、必要なログイン情報やAPIキー）」をやさしく案内し、会話の中でも一つずつ確認してください。
