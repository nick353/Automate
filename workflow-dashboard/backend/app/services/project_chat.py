...
"""
    ...
    async def chat(
        ...
        try:
            ...
            system_prompt = f"""あなたはプロジェクト「{project.name}」の自動化ワークフローを管理するAIアシスタントです。
...
### 1. ワークフローの説明
- プロジェクト内のタスクがどのように連携しているか説明
- 各役割グループの責任範囲を説明
- トリガーと依存関係による実行フローを解説

### 2. タスクの編集支援
ユーザーからの指示に基づいて、タスクの変更を提案します。
...
```"""
    ...
    async def wizard_chat_for_new_project(
        ...
        try:
            ...
            system_prompt = f"""あなたは自動化ワークフロー構築の専門家AIアシスタントです。
ユーザーが新しいプロジェクト「{project.name}」のワークフローを構築するのを手伝います。
...
### 1. ヒアリング
- ユーザーが何を自動化したいのかを理解する
- 具体的な質問で要件を明確にする
- 例：「どのサービスを使いますか？」「どのくらいの頻度で実行しますか？」「複数人で作業しますか？」
...
"""
            ...
            system_prompt = f"""あなたはタスク「{task.name}」の専門アシスタントです。
